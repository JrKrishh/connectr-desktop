...
2. **Pick your tools** — every coding tool on this machine, with whether it is installed
   and signed in. Anything signed out gets a **Sign in** button that opens that tool's own
...
Detection comes from `connectr tools --json`, and creation runs `connectr new` with
the tools you picked. **This app never handles a credential**: signing in happens in
the tool's own CLI, and ConnectR only ever checks whether a credential file exists.
